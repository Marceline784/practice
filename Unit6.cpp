...
if (cartItems.empty()) {
		ShowMessage("Ваш кошик порожній.");
		return;
	}
	...
	try {
		...
		for (auto& item : cartItems) {
			...
			Form9->ADOQuery1->SQL->Text =
				"INSERT INTO Замовлення (Користувач, Напій, Дата, Кількість, Статус) "
				"VALUES (:user, :drink, :date, :qty, :status)";
			Form9->ADOQuery1->Parameters->ParamByName("user")->Value = CurrentUserId;
			Form9->ADOQuery1->Parameters->ParamByName("drink")->Value = item.id;
			Form9->ADOQuery1->Parameters->ParamByName("date")->Value = now;
			Form9->ADOQuery1->Parameters->ParamByName("qty")->Value = item.quantity;
			Form9->ADOQuery1->Parameters->ParamByName("status")->Value = 1;
			Form9->ADOQuery1->ExecSQL();
		}
		...
		ShowMessage("Замовлення оформлено успішно!");
		...
		ShowMessage("Помилка оформлення: " + e.Message);
	}
}
//---------------------------------------------------------------------------
